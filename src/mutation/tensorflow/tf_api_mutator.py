...
from src.mutation import tf_builtin_public_dict
...
class TFAPIMutator(APIMutator):
    def __init__(self,**kwargs):
        framework = kwargs.get("framework", Framework.TENSORFLOW)
        supported_apis = kwargs.get("supported_apis", None)
        super().__init__(framework=framework, supported_apis=supported_apis)
        self.relation_api_mapping = utils.read_pkl(os.path.join(root_path, "data", framework, "tf-relative_api_dict.pkl"))
    ...
    def api_replace_mutation(self,source_code, mutation_point_item, invoke_name2full_name):
        ...
        # public_api_full_name and api_full_name both starts with tensorflow
        # tf_builtin_public_dict and deeprel relation apis starts with tf. so we need to replace tensorflow with tf.
        if api_full_name.replace("tensorflow.", "tf.") not in tf_builtin_public_dict.keys():
            logger.warn(f'{api_full_name.replace("tensorflow.", "tf.")} not in tf_builtin_public_dict')
            return astor.to_source(tree)

        public_api_full_name = tf_builtin_public_dict[api_full_name.replace("tensorflow.", "tf.")].replace("tf.", "tensorflow.")
        ...
        # If the api is torch, it means the api is like `tensorflow.xxx`, we do not extend the similar_root_apis
        # since the apis under torch packages have very different parameters
        if api_root_path != "tensorflow":
        ...
        # TODO: use api_full_name or public_api_full_name?
        ...
        logger.info(f"param_dtype_dict: {old_param_dtype_dict}")

        if public_api_full_name in self.relation_api_mapping.keys() or len(similar_root_apis) != 0:

            # Select the corresponding API in DeepREL and filter for those with constraints
            if public_api_full_name in self.relation_api_mapping.keys():
                relation_apis = self.relation_api_mapping[public_api_full_name]
            else:
                relation_apis = []
            ...
            logger.info(f"api_full_name {api_full_name}")
            logger.info(f"available_relation_apis {available_relation_apis}")
            ...
            for relation_api in available_relation_apis:
                ...
                # Build a parameter dictionary based on the optional parameters of the API such as {'input': 'xxx}
                ...
                if relation_api in similar_root_apis:
                    if "tf." in invoke_name:
                        re_invoke_name = invoke_name.replace("tf.", "tensorflow.")
                    else:
                        re_invoke_name = invoke_name
                    relation_api = relation_api[relation_api.find(re_invoke_name[:re_invoke_name.rfind('.')]):]
                ...
                if not success:
                    logger.warn(f"{relation_api} conversion not supported")
                    continue
            ...
            if len(old_param_dtype_dict.keys()) == 1:
                ...
                if 'return' in signature_data.keys() and \
                    ...
                    logger.debug(f"new_code_replace: {new_code_replace}")
